 ...
 */
if let unwrappedButton = myCastedButtonView {
    unwrappedButton.setTitle("title", for: .normal)
}
 ...
 */
func buttonArray(view: UIView) -> [UIButton] {
    var array = [UIButton]()
...
for item in library {
    if item is Movie {
        ...
        print("Movie: \(movieItem.name), Director: \(movieItem.director)")
    } else if item is Song {
        let songItem = item as! Song
        print("Song: \(songItem.name), Artist: \(songItem.artist)")
    }
}
